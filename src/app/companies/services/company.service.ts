...
export class CompanyService {
  ...
  async createNewCompany(input: CreateCompanyInput) {
    const company = await this.companyRepo.findOne({ where: { name: input.name } })
    if (company) {
      throw new HttpException("Company Name Already Exist", StatusCodeEnum.ALREADY_EXIST)
    }
