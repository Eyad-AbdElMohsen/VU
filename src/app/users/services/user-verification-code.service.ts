...
export class UserVerificationCodeService {
  constructor(
    @InjectRepository(UserVerificationCode) private readonly userVerificationCodeRepo: Repository<UserVerificationCode>,
  ...
  async createVerificationCode(
    ...
    return await this.userVerificationCodeRepo.save({
      ...
      code: process.env.NODE_ENV === 'development' ?
        "1234" : this.helper.createRandomCodeNumber(4),
      expireAt: Date.now() + 15 * 60 * 1000
    })
  }

  async deleteUserVerificationCode(userId: string, useCase: UserVerificationCodeUseCaseEnum) {
