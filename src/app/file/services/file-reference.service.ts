...
export class FileReferenceService {
    ...
    async markFilesAsReferenced(fileIds: string[], modelName: FileModelNameEnum): Promise<void> {
        ...
        for (const file of files) {
            if (file.modelName !== modelName)
                throw new BadRequestException(`File with ID ${file.id} does not belong to model ${modelName}.`);
